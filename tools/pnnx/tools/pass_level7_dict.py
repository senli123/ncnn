...
def run_pass(op,operator_dict, operand_dict):
    # -------run pass------
    ...
    # pass_operators, pass_operands, pass_input_ops, pass_output_ops = export_pnnx(pt_path_str,input_shape_str)
...
if __name__ == "__main__":
    ...
    for pass_file in all_pass_files:
        ...
        while True:
            ...
            for op_name, op in operator_dict.items():
                if op.type == op_type:
                    ...
                    # run_pass(op, operator_dict, operand_dict)
                     # -------run pass------
                    ...
                    # 2. export pnnx
                    # cur_parser = PnnxParser()
                    pass_operators, pass_operands, pass_input_ops, pass_output_ops = parser.getNvpPnnxModel(pass_pt_path, pass_input_shape_str,'None', 'None', op_name)
                    # pass_operators, pass_operands, pass_input_ops, pass_output_ops = export_pnnx(pt_path_str,input_shape_str)
        ...
        if pass_or_not:
            ...
            parser.SaveModel(param_path, visual_operators, visual_operands, 'src')
